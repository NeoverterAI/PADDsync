...
- **Namespace Rebrand**: All references to `Phaser` have been changed to `PADDsync`
...
- **Library Export**: UMD module now exports as `PADDsync` instead of `Phaser`
- **Build Outputs**: All build files use `paddsync` naming convention

### Compatibility

PADDsync maintains full API compatibility with Phaser v3.90.0. All functionality from the original Phaser framework is preserved. To migrate existing Phaser code:
